...
이 레포는 PRD 문서를 바탕으로 바로 확인 가능한 MVP 프로토타입을 포함한다.

| 경로 | 역할 |
| --- | --- |
| `index.html` | 소비자 참여 화면과 운영자 대시보드를 담은 단일 페이지 앱 |
| `src/data.js` | 팝업스토어, 상품, 사전 참여 응답 샘플 데이터 |
| `src/forecast.js` | 방문 수요, 혼잡도, 추천 재고량을 계산하는 규칙 기반 예측 로직 |
| `src/app.js` | 화면 렌더링과 사전 설문 인터랙션 |
| `src/styles.css` | PopCast UI 스타일 |
| `scripts/check-forecast.mjs` | 예측 로직 기본 검증 스크립트 |
| `docs/` | 문제 정의, 시스템 설계, AI 및 데이터 설계, 발표 구성안 |

## 3. 실행 방법

브라우저에서 `index.html`을 직접 열면 프로토타입을 확인할 수 있다.

Node.js가 설치되어 있다면 다음 명령으로 로컬 서버를 띄울 수 있다.

```bash
npm run serve
```

예측 로직만 검증하려면 다음 명령을 실행한다.
